function OnFinish(selProj, selObj)
{
    ...
    try
    {
        ...
        var strTemplateFile = strTemplatePath + "\\WindowsMapFormApplication.vbproj"; 
        ...
        if( project )
        {
            ...
            strTemplateFile = strTemplatePath + "\\MapForm.resx"; 
            item = AddFileToVSProject("MapForm1.resx", project, project.ProjectItems, strTemplateFile, false);
						// Set build action to embedded resource
						item.Properties("BuildAction").Value = 3;


            strTemplateFile = strTemplatePath + "\\MapForm.vb"; 
            item = AddFileToVSProject("MapForm1.vb", project, project.ProjectItems, strTemplateFile, false);
            if( item )
            {
                item.Properties("SubType").Value = "Form";
                project.Properties("StartupObject").Value = project.Properties("RootNamespace").Value + ".MapForm1";
